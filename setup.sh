...
setup_fzf() {
	...
	install_func() {
		...
		[[ ! -e "$fzf_install_script" ]] && {
			log::err "fzf install script not found! Have you cloned the git submodules?"
			return 0
		}
	...
	# Configuration
	configure_func() {
		symlink::safe_create "${SCRIPT_DIR}/modules/fzf/bin/fzf" "${HOME}/.local/bin/fzf"
	}
...
setup_zellij() {
	...
	configure_func() {
		...
		symlink::safe_create "${SCRIPT_DIR}/zellij/plugins" "${HOME}/.local/share/zellij/plugins"
	}
...
setup_diatheke() {
	...
	configure_func() {
		...
		input::prompt_confirmation "Setting up installmgr with remoute sources. Do you want to proceed?" && {
