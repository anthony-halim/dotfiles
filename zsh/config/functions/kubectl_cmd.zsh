...
_fkubectlresourceinfo() {
    ...
    # po    pods
      ...
      set -A "$columns_var" "$name_header,$namespace_header,READY:.status.conditions[?(@.type=='Ready')].type,PHASE:.status.phase,REASON:.status.containerStatuses[?(@.ready==false)].state.*.reason,RESTARTS:.status.containerStatuses[0].restartCount,$creation_header"
